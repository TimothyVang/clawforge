# Product name

**Name:** Clawforge  
**Slug:** `clawforge`  
**Event:** NVIDIA Claw Agent Hackathon  

| Field | Value |
|-------|--------|
| Folder | `projects/clawforge/` |
| GitHub | https://github.com/TimothyVang/clawforge |
| Discord | [HACKATHON TEAM](https://discord.gg/9FFySeV8B) |
| Sandbox | `~/Desktop/hackathon` · browser CDP **9230** |

## Why this name

- **Claw** — Claw Agent Hackathon theme  
- **Forge** — where the crew builds and ships agents  

Tagline: *Forge agents at the Claw hackathon.*
